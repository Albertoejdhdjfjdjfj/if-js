...
function findmatches(string,data){
    return data.filter(item => string.includes(item.name)||string.includes(item.country)||string.includes(item.city));
}
...
function sortcoutryandcity(data){
  ...
  data.forEach(element => {
     if(!res.includes(element.country))
...
function getCalendarMonth(amountinmonth,amountinweek,amountofweek){
  if(amountinmonth<30||amountinmonth>31||amountinweek>7||amountinweek<7||amountofweek>7||amountofweek<1||amountofweek>amountinweek) console.log('Неверно введены данные');
  else{
    let calendaraar=[];
